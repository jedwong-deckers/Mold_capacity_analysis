...
└─────────────────────────────────────────────────────────────┘
                    ...
                    模拟 JS 数据
                              │
                              ▼
              未来后端：Defan 模块 + 脚本引擎 + 数据库
```

> **说明**: 当前 Demo 使用静态 JS 数据；文件上传、校验和计算结果均在浏览器模拟。后端集成不在本 PRD 范围。
...
| Total Molds / 模具总数 | 本任务模具线总数 |
| Matched Molds / 已匹配模具 | 成功匹配的模具线数 |
...
| 枚举值无效 | Brand 不在 UGG/HOKA/TEVA 中；New/Old 不在 New/Old/CO 中；Share/Single 不在允许值中 | "第 [N] 行：[列名] 包含无效值 [X]" |
| Cycle Time 非正数 | Cycle Time ≤ 0 | "第 [N] 行：Cycle Time 必须大于 0" |

**非阻断性警告 / QA 标记**（不阻塞 Next，记录到计算日志和结果中）：

| 校验项 | 规则 | 处理方式 |
|--------|------|----------|
| 非关键列缺失 | 可选列（Size Grading、T3 Ready Date、Sole Special Process Leadtime）为空 | 继续计算；使用默认值或 null |
| 格式自动修正 | Brand 写成 "Hoka" 而非 "HOKA" | 自动转换并记录日志 |
| Multi-supplier QA | 同一 mold+factory+style 映射到多个 supplier | 标记异常行，排除出计算，纳入 QA 报告 |
| 旧模匹配率低 | 已匹配旧模低于阈值（如 70%） | 继续计算，但提示用户核对 Assets Master List 或上传手工匹配 |
| Forecast 款式在 LineSheet 中未找到 | Forecast 中的款式无对应 LineSheet 记录 | 继续计算，使用占位值；标记待数据 review |
...
|------|------|
| Total Molds / 模具总数 | 本任务模具线总数 |
| Matched Molds / 已匹配模具 | 成功匹配到数据源的模具线数 |
...
| Molds Constraint / 受限模具数 | 存在产能约束的模具线数 |
| Mold Constraint Rate / 受限率 | 受限模具/已匹配模具 |
| Total Additional Mold Qty / 总加模数量 | 所有模具线推荐加模数量之和 |
...
> **Demo 说明**: 结果数字为按 taskId（1、2、4）区分的静态模拟数据。
